...
global	I2C_INIT, I2C_write, I2C_READ
...
I2C_INIT:
    ...
    clrf    ANSELC	;Disable Analog functionality of PORTC
    
    clrf    SSPM3	    ;In SSP1CON1
...
I2C_write:
    ...
    btfss   SSP1F		    ; btfss = skip if bit is set, SSPxIF is set by hardware when Start   PIR1, 
    ...
    movff   I2C_DATA, SSP1BUF	    ;The user loads the SSPxBUF with eight bits of data.
    btfsc   ACKSTAT	    ;The MSSPx module shifts in the ACK bit from    in SSP1CON2
    bra	    $-2			    ;the slave device and writes its value into the
				    ;ACKSTAT bit of the SSPxCON2 register.			    
    btfss   SSP1IF	    ;The MSSPx module generates an interrupt at the end      , in PIR1
...
I2C_READ:    
    ...
    movff   I2C_ADDRESS, SSP1BUF    ;The user loads the SSPxBUF with the slave address to transmit
    btfsc   ACKSTAT	    ;The MSSPx module shifts in the ACK bit from    SSP1CON2, 
    bra	    $-2			    ;the slave device and writes its value into the
				    ;ACKSTAT bit of the SSPxCON2 register.
    btfss   SSP1IF	    ;The MSSPx module generates an interrupt at the end    PIR1, 
    ...
    MOVFF   I2C_DATA	    ;and reads the received byte from SSPxUF    SSP1BUF, 
